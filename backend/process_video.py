...
def process_video(input_path: Path, output_path: Path, fps: float | None = None) -> Path:
    """Convert one video into a deterministic, offline Mixamo JSON clip."""
    ...
    clip = convert_frames_to_mixamo_clip(cleaned, fps=target_fps)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Convert a 2D video into a Mixamo animation JSON clip.")
    parser.add_argument("input", type=Path, help="Input MP4/MOV video")
    parser.add_argument("output", type=Path, help="Output JSON clip")
    parser.add_argument("--fps", type=float, default=None, help="Optional output FPS; defaults to source FPS")
    args = parser.parse_args()
    result = process_video(args.input, args.output, args.fps)
